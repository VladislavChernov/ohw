...
Версии зафиксированы: образ `ollama/ollama:0.32.15` (не `latest`), модель
`qwen2.5:7b-instruct` (тег по умолчанию, переопределяется `OLLAMA_MODEL`).
Python в базовом образе запинен точно: `python:3.13.5-slim-bookworm`
(см. `python/Dockerfile`); в проектах — `.python-version = 3.13.5` (uv).
...
`<local-path>`) и синхронизируется сюда; такие пути machine-specific и в
