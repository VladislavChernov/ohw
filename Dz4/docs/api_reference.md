...
| Код  | Описание |
|------|----------|
| `400 Bad Request` | Невалидный JSON/тело, нарушение синтаксиса YAML-профиля, профиль не является YAML-маппингом |
| `401 Unauthorized` | Сервис требует `X-API-Key`, ключ не передан или неверен |
| `422 Unprocessable Entity` | Профиль структурно некорректен: отсутствует обязательная секция, `ontology.node_types`/`edge_types` не список, секция не маппинг, нарушена однозначность глоссария; либо `profile.name` не совпадает с доменом при активации |
