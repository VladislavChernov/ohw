# Документация: Operations, Масштабирования и Матрица Рисков (v4.0 — АРХИВ)
...
- `graphrap_dedup_merges_total{stage="auto|llm"}` — Счетчик авто- и ЛЛМ-мержей
- `graphrap_validation_errors_total{type, rule, domain}` — Ошибки (structural/semantic)
- `graphrap_canonicalization_total{layer, domain}` — Статистика слоёв нормализатора
- `graphrap_canonicalization_fallback_total{domain}` — Счетчик вызовов ЛЛМ-fallback
- `graphrap_domain_switch_total` — Метрика частоты смены доменов
...
Примечание: Ниже зафиксированы 5 критических инфраструктурных рисков текущей фазы. Полный расширенный реестр (из 12 рисков концепта, включая ложные склейки и качество экстракции) вынесен в отдельный проектный документ "Full_Risk_Registry.txt".
