...
> **Версия:** v7.2 (итерация поверх базы v5; L2-07 → planned, L4-04 → partial)
...
| ID | Инвариант | Обоснование |
|----|-----------|-------------|
| L4-01 | GPU-инвариант: эмбеддинги и LLM разделяют одну видеокарту → их работа строго поочерёдна через гейтинг по фазам (композ-профили); параллельный инференс двух моделей на одном GPU запрещён *(кандидат переквалификации в ограничение среды на стадии «Рост» — ADR-027, решение отложено)* | `docs/06` §3, §5 (риск №1) |
