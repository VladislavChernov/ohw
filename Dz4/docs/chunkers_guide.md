...
- CHUNK — обязательный этап ядра: document → chunk → embed → vector commit (L3-01: graph projection
  опционален и не блокирует success, но chunk пропустить нельзя). Чанкинг нельзя вынести
