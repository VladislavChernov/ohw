...
4. Что пайплайн из 9 этапов (INGEST → … → COMMIT) реально исполним на целевом железе с
   учётом GPU-гейтинга (поочерёдная работа эмбеддингов bge-m3 и LLM Qwen 7B — ограничение
   среды прототипа, ADR-027; «L4-01» сейчас = контракт `LLMInference`, см. `invariants.md`).
...
| GPU | NVIDIA, 8 ГБ VRAM (RTX 2070 Super и выше) | bge-m3 + Qwen 7B поочерёдно (гейтинг L4-01, ограничение среды — ADR-027) |
