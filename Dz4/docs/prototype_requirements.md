...
|-----------|------|-----------|-------------------|
| `INGEST_MAX_CONCURRENT=1` | Сериализация ingestion-джоб для обхода Neo4j deadlock при параллельной записи в общие сущности (ADR-027-паттерн: ограничение среды, не инвариант) | После S2 из бандла `concurrent-ingest-write-policy` (retry transient + детерминированный порядок) | Очередь ingestion (Redis/RabbitMQ → Kafka, `docs/06` §4) |
