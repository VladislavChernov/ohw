...
> История: блоки-срезы v4 и отдельная схема v5 не поддерживаются; исторические копии — в git-истории (архивный снапшот `docs.zip` v4/v5-эпохи восстановим из коммита `8a83c72`) и `docs/history.md`. Детализация текущей итерации — в этом файле (Mermaid) и `docs/05_adr_log.md` (ADR-013).
...
3. Новая итерация концепции замещает актуальную схему целиком; прежние срезы не поддерживаются и живут только в истории (`docs/history.md` и git-коммиты).