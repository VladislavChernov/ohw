...
- [x] 1.2. `docs/06_operations_and_risks.md` §5: добавить риск «Neo4j deadlock при
      INGEST_MAX_CONCURRENT>1» с митигацией (S1: сериализация; S2: retry+sort).
      Выполнено: блок «Риск №7 (S1-обход, закрыт S2 — ADR-028)» в `docs/06` §5.
- [x] 1.3. Полный прогон M5 (вариант 1 и 2) проходит на концентрации 1 (фиксация результата
      в lift_report + история).
      Выполнено: история «Этап 14» (`docs/history.md`) фиксирует S1-обход M5 на конц=1;
      финальный архивный lift_report M5 (конц=2) — `prototype/reports/s2-live/m5/lift_report.json`
      (created_at 2026-09-19, 50 вопросов, verdict pass). Отдельный отчёт прогона при
      конц=1 в репо не сохранялся.
...
- [x] 2.5.7. Интеграция: контрактные тесты Neo4j-адаптера — transient-классификация
      на реальном драйвере (если доступен стек).
      Выполнено live-прогоном `dz4_live_transient.py` (2026-09-20, ALL CHECKS PASSED);
      временный скрипт не сохранён в репо.

### 2.6. Приёмка

- [x] 2.6.1. Live-стек Neo4j: полный прогон M5 с `INGEST_MAX_CONCURRENT=2` без deadlock;
      повторная загрузка — no-op (L2-06).
      Выполнено: M5 на конц=2 без deadlock (s2-live/m5). Проверка no-op (L2-06)
      перенесена в `eval-graph-contribution-experiment` 1.3 — там же код EXTRACT
      меняется на типизированный, ревизия корпуса сменится после переингеста.
- [x] 2.6.2. Ретракт дефолта: `compose.eval.yaml` возвращается к 2 (после успешного прогона),
      комментарий S1 снимается.
      Выполнено: `INGEST_MAX_CONCURRENT: 2` (2026-09-22); пометка «S1-обход» в самом
      compose-файле отсутствовала (task 1.1 фиксировала проверку `docker inspect`),
      комментарий снимать не понадобилось.
