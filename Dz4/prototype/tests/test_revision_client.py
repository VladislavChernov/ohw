...
def _stub_get(monkeypatch: pytest.MonkeyPatch, seq: list[Any]) -> Any:
...
def test_revision_fetch_and_cache(monkeypatch: pytest.MonkeyPatch) -> None:
    fake = _stub_get(
        monkeypatch,
        [
            {"revision": VALID_REV, "updated_at": "2026-09-15T10:00:00Z"},
            {"revision": VALID_REV_2, "updated_at": "2026-09-15T10:00:01Z"},
        ],
    )
    client = RevisionClient("http://ing:8002", poll_interval_s=60.0)
    assert client.revision("it") == VALID_REV
    assert client.revision("it") == VALID_REV  # в окне -> из кэша, без второго GET
    assert len(fake.calls) == 1
    assert client.known_revisions() == {"it": VALID_REV}
...
def test_revision_per_domain_cache(monkeypatch: pytest.MonkeyPatch) -> None:
    fake = _stub_get(
        monkeypatch,
        [
            {"revision": VALID_REV, "updated_at": None},
            {"revision": VALID_REV_3, "updated_at": None},
        ],
    )
    client = RevisionClient("http://ing:8002", poll_interval_s=60.0)
    assert client.revision("it") == VALID_REV
    assert client.revision("cpp") == VALID_REV_3
    assert len(fake.calls) == 2
    assert client.known_revisions() == {"it": VALID_REV, "cpp": VALID_REV_3}
...
def test_revision_fail_open_last_known(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert client.revision("it") == VALID_REV
    time.sleep(0.06)
    assert client.revision("it") == VALID_REV  # fail-open: последняя известная
    assert client.revision_poll_errors_total == 1
    assert client.known_revisions() == {"it": VALID_REV}


def test_revision_empty_value_is_rejected(monkeypatch: pytest.MonkeyPatch) -> None:
    _stub_get(monkeypatch, [{"revision": "", "updated_at": None}])
    client = RevisionClient("http://ing:8002", poll_interval_s=5.0)
    assert client.revision("it") is None
    assert client.revision_poll_errors_total == 1


def test_revision_non_hex_response_fails_open(monkeypatch: pytest.MonkeyPatch) -> None:
    _stub_get(monkeypatch, [{"revision": "not-hex", "updated_at": None}])
    client = RevisionClient("http://ing:8002", poll_interval_s=5.0)
    assert client.revision("it") is None
    assert client.revision_poll_errors_total == 1


def test_revision_missing_field_fails_open(monkeypatch: pytest.MonkeyPatch) -> None:
    _stub_get(monkeypatch, [{"updated_at": None}])
    client = RevisionClient("http://ing:8002", poll_interval_s=5.0)
    assert client.revision("it") is None
    assert client.revision_poll_errors_total == 1


def test_revision_non_mapping_response_fails_open(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_revision_disabled_returns_none(monkeypatch: pytest.MonkeyPatch) -> None:
    fake = _stub_get(monkeypatch, [{"revision": VALID_REV, "updated_at": None}])
