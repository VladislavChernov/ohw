...
поэтому одна сущность из разных документов домена — один узел, и атрибуция оси идёт через
`ContextNode.source_ids` (документы не содержат эту связь явно, поэтому векторный поиск по тексту
вопроса их не ранжирует). Но что именно извлечёт LLM при `EXTRACT_LLM=true`, известно только после
прогона, поэтому `necessity` для этих вопросов заранее не подтверждена.

Линтер проверяет только количество и флаги, поэтому «зелёный» датасет не означает, что вклад графа
измеряется. Проверять надо по артефактам, и разбор однозначен:

| Признак в `qa_review.md` / `results.jsonl` | Что это значит |
|---|---|
| `graph_paths` пуст, `graph_degraded: true` | дефект стенда, а не вопрос |
| `graph_paths` непуст, `necessity = 0` | вопрос не требует графа: обход ничего не добавил сверх вектора |
| `recall@5` низкий, `groundedness` высокий | система ответила верно, но не на заданный вопрос |
| `coverage` низкий, `groundedness` высокий | `golden_facts` неполны или неверны |
| `golden_coverage` в манифесте ниже 1.0 | часть `golden_sources` недостижима — сначала исправить эталон |

Вопрос, признанный по этим признакам плохим, заменяется, а не «чинится» настройкой прогона.
