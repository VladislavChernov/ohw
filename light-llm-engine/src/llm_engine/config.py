"""Configuration: defaults < TOML file < env OLLAMA_* < CLI arguments."""
...
def load_config(explicit_path: Path | None = None) -> Config:
    ...
    try:
        ...
        raise ConfigError(f"cannot decode config {path}: {exc}") from exc
    ...
    return Config(
        ...
        system=_string(ollama, "system", "", path),
...
def _section(raw: dict, name: str, path: Path) -> dict:
    ...
    if unknown:
        print(
            f"warning: {path}: ignoring unknown keys in [{name}]: {', '.join(sorted(unknown))}",
            flush=True,
        )
    return section
