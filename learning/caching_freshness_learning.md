...
кэш в query-контуре, `data_revision_analytics.md`). Часть распределённых
...
- Это прямого «стандартного учебника» нет — идея приходит из практики:
  ...
  - **Merkle tree** — https://en.wikipedia.org/wiki/Merkle_tree
    (наш `sha256(пары source_url + content_hash)` — плоская мини-версия дерева Меркла).
...
> **Продолжение темы.** Этот файл — про **свежесть и инвалидацию** («насколько
> стар ответ»). Про **выбор метода** (что/где/сколько кэшировать, семантика
> отказа, конкурентность, экономика) — отдельная методичка
> [`caching_method_selection_learning.md`](caching_method_selection_learning.md).
...
и что меняет введение ревизии в инвариант L2-07.
...
`data_revision_analytics.md`, если что-то стало яснее/глубже.
