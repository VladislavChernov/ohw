...
| Модуль | Роль |
|---|---|
| `core.py` | HTTP-ядро: исполнение шагов, `{var}`-подстановка, `extract` (JSONPath), `on_fail`, `cleanup`, оценка `expect` через **`ohw_kit.checks.evaluate`** |
...
должны быть заданы в `vars` или извлечены через `extract` в steps.
...
```powershell
uv run pytest -q      # 39 тестов (MockTransport, без сети)
