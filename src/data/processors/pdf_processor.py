...
def process_single_file(file_path: str) -> tuple[str, str]:
    ...
    print(f"[Processor] Обработка PDF-файла: {os.path.basename(file_path)}")
    # *** TODO: Реализовать парсинг PDF и вернуть извлеченный текст ***
    return f"PDF Placeholder Content from {os.path.basename(file_path)}", os.path.basename(file_path)
